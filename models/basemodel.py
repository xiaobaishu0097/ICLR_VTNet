...
class BaseModel(torch.nn.Module):
    def __init__(self, args):
        ...
        if self.detection_alg == 'fasterrcnn':
            ...
            self.detection_feature = nn.Linear(263, 49)
            self.num_cate = 100
        elif self.detection_alg == 'fasterrcnn_bottom':
            self.detection_feature = nn.Linear(262, 49)
    ...
    def embedding(self, state, detection_inputs, action_embedding_input):
        if self.detection_alg == 'fasterrcnn':
            detection_input = torch.cat((detection_inputs['features'], detection_inputs['bboxes'],
                                         detection_inputs['scores'].unsqueeze(dim=1), detection_inputs['indicator']),
                                        dim=1)
        elif self.detection_alg == 'fasterrcnn_bottom':
            detection_input = torch.cat((detection_inputs['features'], detection_inputs['bboxes'],
                                         detection_inputs['scores'].unsqueeze(dim=1), detection_inputs['indicator']),
            ...
            detection_input = torch.cat(
                (detection_inputs['features'], detection_inputs['bboxes'], detection_inputs['labels'].unsqueeze(dim=1),
                 detection_inputs['scores'].unsqueeze(dim=1), detection_inputs['indicator']),
                dim=1)

        detection_input = F.relu(self.detection_feature(detection_input))
        detection_input = detection_input.reshape(1, self.num_cate, 7, 7)
        ...
        x = torch.cat((x, detection_input, action_reshaped), dim=1)
